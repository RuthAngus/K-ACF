# This version of the ACF code is designed to run CoRoT data (specifically the one lc provided by Hannu)
...
def corr_run(tr_out = False, tr_type = None):
    ...
    lc_file = ('/Users/angusr/angusr/CoRoT/C27b_lc.txt') 
    print 'star = ', lc_file
    ...
    pylab.savefig('%s/CoRoT/plots_pgram/%s_pgram.png' \
                  %(dir, x))
    ...
    savefilen = '%s/CoRoT/results.txt' % (dir)
...
def acf_calc(time, flux, interval, kid, max_psearch_len):
    ...
    pylab.savefig('%s/CoRoT/plots_sm/%s_sm.png' % (dir, kid))
