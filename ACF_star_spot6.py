# This version of the ACF code is designed to run on the data generated by spot_sim_ruth.py
# ACF_star_spot5 is for running on the 'test' files
# ACF_star_spot6 names stars as numbers not strings
...
def all_stars():
	lcfile = numpy.array(glob.glob('/Users/angusr/angusr/ACF/star_spot_sim/sim_?????.png.mat'))
	for i in lcfile:
		corr_run(i, 'ss11-14')
		raw_input('enter')
	return
...
def corr_run(quarter_sel, quarter_sel2, tr_out = False, tr_type = None, number_of_stars = 24):

    #read in index and create list of unique KIDs
    index = atpy.Table('/Users/angusr/angusr/ACF/index_Q%stest.ipac' %quarter_sel, type = 'ipac')

    print '/Users/angusr/angusr/ACF/index_Q%s.ipac' %quarter_sel
    print 'Index includes %s files...'% len(index)
    #id_list = scipy.array(list(Set(index.keplerid)))
    #id_list = scipy.array(range(1,25))
    id_list = scipy.array(range(1,number_of_stars))
    print '...and %d individual stars' % len(id_list)
    id_list.sort()
    t = index.where(index.quarter == 0)
    t.sort('keplerid')

    t.remove_columns(['long_cadence'])
    t.remove_columns(['quarter'])
    t.remove_columns(['output'])
    t.remove_columns(['module'])
    t.remove_columns(['filename'])
    t.remove_columns(['mtime'])
    t.remove_columns(['pmtotal'])
    t.remove_columns(['glon'])
    t.remove_columns(['glat'])
    t.remove_columns(['variable'])
    t.remove_columns(['galaxy'])
    t.remove_columns(['aq'])
    t.remove_columns(['rad'])
    t.remove_columns(['av'])
    t.remove_columns(['ebv'])
    t.remove_columns(['feh'])
    t.remove_columns(['logg'])
    t.remove_columns(['teff'])
    t.remove_columns(['gkcol'])
    t.remove_columns(['kepmag'])
    t.remove_columns(['kmag'])
    t.remove_columns(['hmag'])
    t.remove_columns(['jmag'])
    t.remove_columns(['zmag'])
    t.remove_columns(['imag'])
    t.remove_columns(['rmag'])
    t.remove_columns(['gmag'])
    t.remove_columns(['pmdec'])
    t.remove_columns(['pmra'])
    t.remove_columns(['dec'])
    t.remove_columns(['ra'])
    t.remove_columns(['tmid'])

    #savefilen = '%s/PDCQ%s_output/test.txt' % (dir,quarter_sel)
    #atpy.asciitables.write_ascii(t, savefilen, delimiter = ' ', overwrite = True)
       
    #time = time + 260.244739529
    
    #id_list = [1]
    print 'found %d individual stars' % len(id_list)
    ...
    for w in range(0,24):
	    for y in range(0, len(names)):
		    for z in range(0, len(Amps)):
			    ...
			    #read in index and create list of unique KIDs
			    mat = scipy.io.loadmat('%s' %lc_file)
			    lc =  mat['ts']
			    time = lc[0]
			    flux = lc[2]
			    mdn = numpy.median(flux)
			    flux = flux-mdn
			    lc_tab = atpy.Table()
    ...
    #print type(period)
    #my_KID_list = numpy.array(my_KID_list)
    #print type(my_KID_list)
    #print len(period)
    ...
    # numpy.savetxt('%s/PDCQ%s_output/new_results.txt' % (dir,quarter_sel), \
    # 		  numpy.transpose((my_KID_list, my_period_list, my_error_list, my_sine_list)))
    
    #return kid_x, period, dlag_per_err, sine_per
    return
